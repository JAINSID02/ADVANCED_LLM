...
def load_training_data(alpaca_split: str = "train[:3000]", include_chitchat: bool = True,
                        chitchat_repeat: int = 3, include_about_me: bool = False,
                        about_me_repeat: int = 1, include_alpaca: bool = True) -> List[SFTItem]:
    items = load_tiny_hf(split=alpaca_split, sample_dataset=False) if include_alpaca else []
    ...
    if include_about_me:
        from about_me_data import load_about_me
        items = items + load_about_me(repeat=about_me_repeat)
