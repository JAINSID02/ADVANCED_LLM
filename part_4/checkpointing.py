    ...
    try :
        ...
        with torch.no_grad() , torch.amp.autocast(device_type=xb.device.type,enabled=False):
            ...
            for li , blk in enumerate(getattr(model,"blocks",[])):
                ...
                k=attn.wk(h).view(B,T,attn.n_head,attn.d_head).transpose(1,2) #(B,Hk,T,D)
                v=attn.wv(h).view(B,T,attn.n_head,attn.d_head).transpose(1,2) #(B,Hk,T,D)

                # take a tiny slice to keeps logs light

                q1 =  q[:1,:1].contiguous().view(-1).float().cpu()
                k1 =  k[:1,:1].contiguous().view(-1).float().cpu()
                v1 =  v[:1,:1].contiguous().view(-1).float().cpu()

                # drop no finite defensive
