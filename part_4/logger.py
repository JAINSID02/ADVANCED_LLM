...
class TBLogger(NoopLogger):
    ...
    def log (self  , step : Optional[int] = None  , **kv:Any):
        ...
        for k,v in kv.items():
            ...
            #scalar vs histogram autotune

            try:
                import torch , numpy as np
                is_torch = isinstance(v,torch.Tensor)
                is_np = isinstance(v , np.darray)
    ...
    def image(self , tag:str  , img , step : Optional[int] = None):
        ...
        try :
            self.w.add_image(tag , img  ,global_step = step , detaformats = "CHW" if getattr(img , "ndim" , 0) == 3 and img.shape in [1,3] else "HWC")
...
def init_logger(which : str  , out_dir : str = "runs/part4"):
    ...
    if which == 'wandb':
        return WBLogger(project = 'llm_part4')
    return NoopLogger()
