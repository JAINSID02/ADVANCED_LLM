...
class GPTModern(nn.Module):
    def __init__(self , vocab_size : int  = 256, block_size : int = 256,
        ...
        self.blocks=nn.ModuleList([TransformerBlockModern(n_embd,n_head,dropout,use_rmsnorm,use_swiglu,rope,max_pos,sliding_window,
                                                          attention_sink,n_kv_head)for _ in range(n_layer)])
    ...
    @torch.no_grad()    
    def generate(self , prompt : torch.Tensor , max_new_tokens = 200  , temperature = 1.0 , top_k =50,
                 top_p=None,eos_id=1,sliding_window:int|None=None,attention_sink:int=0)  : 
        try :
